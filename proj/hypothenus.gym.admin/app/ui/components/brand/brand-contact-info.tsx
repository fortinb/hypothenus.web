...
export default function BrandContactInfo({ isEditMode }: { isEditMode: boolean }) {
                ...
                <Accordion defaultActiveKey={formContacts.fields?.map((_: Record<string, any>, index: number) => index.toString())} alwaysOpen>
                    ...
                    {formContacts.fields?.map((contact: Record<string, any>, index: number) => {
                        return <Accordion.Item key={index} eventKey={index.toString()} className="pt-2">
                            <Accordion.Header data-testid={`brand-contact-accordion-${index}`} className={"accordion-header-light" + (getError(index) ? " accordeon-header-invalid" : "")}>{formatContactName(contact as Contact)}</Accordion.Header>
                            <Accordion.Body className="p-0">
                                <Card>
                                    <Card.Body className="">
                                        ...
                                        <ContactInfo key={contact.id} index={index} id={`brand_contact_${index}`} formStatefield={`contacts.${index}`} />
                                    </Card.Body>
                                </Card>
                            </Accordion.Body>
                        </Accordion.Item>
