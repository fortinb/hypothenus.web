    ...
    return (
        <Row className="gx-2 py-2">
            ...
            <Col xs={3} className="d-flex align-items-center">
                <Row className="gx-2 d-flex flex-row flex-fill justify-content-center">
                    <Col xs={4} className="d-flex flex-column justify-content-center">
                        <Button className="btn btn-icon btn-sm" disabled={!isEditMode || item.quantity <= 1} title={t("cart.item.decreaseQuantity")}
                    ...
                    <Col xs={4} className="d-flex flex-column justify-content-center">
                        <Button className="btn btn-icon btn-sm" disabled={!isEditMode} title={t("cart.item.increaseQuantity")} onClick={() => onUpdateQuantity(item.membershipPlan.uuid, item.quantity + 1)}>
            ...
            <Col xs={1} className="d-flex flex-column justify-content-center">
                <Button className="btn btn-icon btn-sm" onClick={() => onRemove(item.membershipPlan.uuid)} title={t("cart.item.remove")} disabled={!isEditMode}>
