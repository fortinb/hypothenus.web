...
export default function MembershipPlanCardInfo({ membershipPlan, tLocale, lang, onlyDisplay, linkActive, linkUri, onMembershipPlanAction }: {
    ...
    linkActive: boolean;
    linkUri: string;
    onMembershipPlanAction: (addToCart: boolean, buyNow: boolean, membershipPlan: MembershipPlan) => void;
}) {
    const router = useRouter();
    const [showMembershipPlanDetailModal, setShowMembershipPlanDetailModal] = useState(false);

    function handleMembershipPlanModalResult(addToCart: boolean, buyNow: boolean, membershipPlan: MembershipPlan) {
        setShowMembershipPlanDetailModal(false);
        onMembershipPlanAction(addToCart, buyNow, membershipPlan);
    }
    ...
    return (
        <>
                        ...
                        <div className="d-flex flex-column justify-content-center mb-3">
                            ...
                            <Card.Text>
                                <>
                                    ...
                                    <span className="my-2 card-text-larger">{getMembershipPlanPrice(membershipPlan, true)} </span>
            ...
            <ModalBuyMembershipPlan membershipPlan={membershipPlan} tLocale={tLocale} onlyDisplay={onlyDisplay} locale={lang}
                isAction={false} show={showMembershipPlanDetailModal} handleResult={handleMembershipPlanModalResult} />
        </>
    );
}
